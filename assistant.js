...
// --- CONFIGURATION (DIGITAL FORTRESS v9.9 FLASH) ---
const CONFIG = {
...
const MAX_AI_CALLS_PER_MIN = 12; // Лимит Flash позволяет делать много запросов
...
const connectDB = async () => {
    try {
        ...
        console.log("✅ [NEURAL TITAN]: Fortress Engine v9.9 Digital Online (FLASH)");
        syncWithDatabase();
    } catch (err) { 
...
// --- BINANCE WS ENGINE (ПРОФЕССИОНАЛЬНАЯ СТАБИЛИЗАЦИЯ) ---
function initTitanStream() {
    ...
    ws.on('open', () => { console.log("📡 [Binance WS]: Connected to Market Flow."); });
...
// --- УЛУЧШЕННЫЙ РАСЧЕТ ОЧКОВ (В USDT ДЛЯ ТОЧНОСТИ) ---
function updateTechnicalScore(coinId, price) {
    ...
    // 2. ABSORPTION ($150,000 threshold)
...
// --- ЦЕНТРАЛЬНЫЙ СКАНЕР (ОПТИМИЗИРОВАН ДЛЯ FLASH) ---
    ...
    try {
        ...
        for (const v of verdicts) {
            ...
            if (v.action.includes('EXECUTE') && v.confidence >= 80 && !activeMasterSignals[v.id]) {
                ...
                // Автоматический расчет TP/SL если ИИ вернул 0
                let tp = v.tp, sl = v.sl;
                if (tp === 0 || sl === 0) {
