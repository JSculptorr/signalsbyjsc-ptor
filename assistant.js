...
// --- 🧠 МОДУЛЬ ИИ И САМООБУЧЕНИЯ (SELF-LEARNING) ---
async function runMarketScan() {
    ...
    try {
        const recentTrades = await Trade.find().sort({ timestamp: -1 }).limit(5); // Берем последние 5 сделок
    ...
    try {
        const prompt = `ACT AS SMC PRO INSTITUTIONAL TRADER. Analyze OHLCV candle arrays.
        Identify: Market Trend, Order Blocks, Fair Value Gaps.

        [SELF-LEARNING MEMORY - Твои последние 5 сделок]:
        ${learningContext}
        Если в истории есть убыточные сделки (FAILED), проанализируй, почему твоя уверенность была ложной, и скорректируй свою логику. Если сделки успешные (SUCCESS) - используй этот же паттерн для поиска новых.

        Data: ${JSON.stringify(batchData)}
        
        OUTPUT ONLY JSON ARRAY:
        [{"id": "BTC", "action": "EXECUTE_LONG", "reason": "BOS detected after sweep", "confidence": 85, "tp": 0, "sl": 0}, ...]
...
// 🚀 Сняли ошейник по времени. Теперь сканирует каждые 10 секунд!
...
async function finalizeMasterTrade(coinId, result, exitPrice) {
    ...
    const tradeDoc = new Trade({
        userId: null, // Системный трейд для обучения
