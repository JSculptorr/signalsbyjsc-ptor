...
const CONFIG = {
    ...
    // 🎯 ПЕРЕКЛЮЧИЛИСЬ НА PRO МОДЕЛЬ ДЛЯ ОБХОДА ПЕРЕГРУЗОК
...
const connectDB = async () => {
    try {
        ...
        console.log("✅ [NEURAL TITAN]: Fortress Engine v9.9 Digital Online (IMMORTAL PRO)");
        syncWithDatabase();
    } catch (err) { 
...
async function runMarketScan() {
    ...
    // 🛡️ СИСТЕМА АВТО-ТАРАНА: 3 попытки с интервалом в 3 секунды
    let maxRetries = 3;
    let attempt = 0;
    let success = false;
    ...
    while (attempt < maxRetries && !success) {
        try {
            ...
            text = response.text();
            success = true; // Если дошли сюда, значит ИИ ответил без 503 ошибки
        } catch (e) {
            attempt++;
            if (e.message.includes('503') && attempt < maxRetries) {
                console.log(`⚠️ [AI Engine]: Сервер Google перегружен (503). Попытка ${attempt}/${maxRetries} через 3 секунды...`);
                await new Promise(r => setTimeout(r, 3000));
            } else {
                console.error(`❌ [AI Error]: Критическая ошибка Gemini API: ${e.message}`);
                broadcastHackerLog(`[System]: Сбой нейросети (${e.message.substring(0, 20)}...)`, 'ALERT');
                return; // Выходим из функции, если это не 503 или попытки кончились
            }
        }
    }

    if (!success) return; // Страховка
