...
const User = mongoose.model('User', new mongoose.Schema({
    ...
    balance: { type: Number, default: 10000 }, // Виртуальный баланс для расчета 1% риска
...
const ActiveSignal = mongoose.model('ActiveSignal', new mongoose.Schema({
    ...
    sl: Number,
    tp: Number,
    size: Number, // Объем позиции для 1% риска
    partialHit: { type: Boolean, default: false }, // Флаг фиксации 1R
    desc: String,
...
const Trade = mongoose.model('Trade', new mongoose.Schema({
    ...
    tp: Number,
    result: String, 
    profitCash: Number, // Прибыль в $
    rr: Number, // Соотношение Риск/Прибыль
    timestamp: { type: Date, default: Date.now }
}));
...
let antiTilt = { BTC: { losses: 0, pauseUntil: 0 }, ETH: { losses: 0, pauseUntil: 0 }, SOL: { losses: 0, pauseUntil: 0 } };
...
async function calculateMetrics() {
    ...
    stats = {
        ...
        avgRR: (totalRR / allTrades.length).toFixed(1),
...
function isTradeSession() {
    ...
    return hour >= 8 && hour <= 21; // London + NY Sessions
}
...
function findInstitutionalOB(ticks, type) {
    ...
    for (let i = ticks.length - 10; i < ticks.length - 2; i++) {
        ...
        const displacement = Math.abs(next[4] - next[1]) > avgBody * 2.2; // Импульсный выход
        const fvg = type === 'LONG' ? ticks[i+2][3] > curr[2] : ticks[i+2][2] < curr[3]; // Наличие имбаланса
...
async function analyzeSMC_Ultimate(coin, h1ticks) {
    if (activeSignals[coin.id] || Date.now() < antiTilt[coin.id].pauseUntil) return;
    ...
    try {
        ...
        if (findLiquiditySweep(h1ticks, side)) score += 4;
        const ob = findInstitutionalOB(h1ticks, side);
        if (ob) score += 3;
        // FVG Check
        if (side === 'LONG' && h1ticks[h1ticks.length-3][2] < h1ticks[h1ticks.length-1][3]) score += 3;
        if (side === 'SHORT' && h1ticks[h1ticks.length-3][3] > h1ticks[h1ticks.length-1][2]) score += 3;
        ...
        if (score >= 8) {
            ...
            if (confirmed) {
                ...
                const newSig = new ActiveSignal({
                    ...
                    desc: `1% RISK | OB + DISPLACEMENT | CONFIRMED`
                });
                await newSig.save();
                activeSignals[coin.id] = { ...newSig._doc, status: "active" };
                console.log(`🎯 [ENTRY]: ${coin.id} с риском 1% ($${riskPerTrade.toFixed(2)})`);
            }
        }
    } catch (e) { console.error("Ошибка анализа"); }
}
...
async function checkTradeExecution(coin, price) {
    ...
    // 1R Target для частичного закрытия
    const target1R = isLong ? entry + (entry - sl) : entry - (sl - entry);

    // Логика Partial Close (50% на 1R) + BE
    if (!sig.partialHit) {
        if ((isLong && price >= target1R) || (!isLong && price <= target1R)) {
            ...
            activeSignals[coin.id].sl = entry; // Перевод в безубыток
            console.log(`💰 [PARTIAL]: 1R достигнут по ${coin.id}. 50% в сейфе, остальное в БУ.`);
        }
    }
...
async function finalizeTrade(coinId, result, exitPrice) {
    ...
    // Расчет профита с учетом частичного закрытия
    let profitCash = (exitPrice - entry) * sig.size;
    if (sig.partialHit && result === "FAILED") profitCash = (entry - entry) * sig.size; // Выход в 0 (БУ)
...
app.listen(PORT, () => {
    console.log(`\n=========================================`);
    console.log(` >>> JSCULPTOR SURGEON ULTIMATE 10/10 <<< `);
    console.log(`=========================================`);
    console.log(`[OK] 1% RISK MGMT & POSITION SIZING: ACTIVE`);
    console.log(`[OK] PARTIAL TAKE PROFIT (1R) & BE: ACTIVE`);
    console.log(`[OK] ADVANCED PERFORMANCE METRICS: ONLINE`);
    console.log(`=========================================\n`);
});