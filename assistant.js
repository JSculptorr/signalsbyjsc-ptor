...
async function finalizeTrade(coinId, result, exitPrice) {
    ...
    // Находим владельца сигнала или просто активного пользователя для обновления баланса в БД
...
// ГЛАВНЫЙ ЭНДОПОИНТ ДАННЫХ (ОБНОВЛЕН ДЛЯ РЕАЛЬНОГО БАЛАНСА)
app.get('/api/data', async (req, res) => {
    ...
    if (auth && auth !== 'Bearer null') {
        try {
            ...
            if (user) {
                const now = new Date();
                if (user.subscriptionStatus === "active" || (user.subscriptionExpires && user.subscriptionExpires > now)) {
                    isPrem = true;
                }
                autoMode = user.autoTrade;

                // Получение реального баланса именно для ЭТОГО пользователя
                if (user.binanceKey && user.binanceSecret) {
                    try {
                        ...
                        if (Array.isArray(accBalance)) {
                            const usdt = accBalance.find(b => b.asset === 'USDT');
                            if (usdt) {
                                realBalance = parseFloat(usdt.balance).toFixed(2);
                                // Синхронизируем баланс в базе данных
                                user.balance = parseFloat(realBalance);
                                await user.save();
                            }
                        }
                    } catch (err) { 
                        console.log(`Binance Balance Error for ${user.email}:`, err.message); 
                    }
                }
            }
        } catch (e) { console.error("Auth Data Error:", e.message); }
    }
