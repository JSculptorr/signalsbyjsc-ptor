...
// --- МОДЕЛИ ДАННЫХ (БЕЗ ИЗМЕНЕНИЙ СТРУКТУРЫ) ---
...
// --- ЛОГИКА СИГНАЛОВ И ИСТОРИИ (10 ДНЕЙ) ---
let lastPrice = 0;
...
function pushToHistory(result) {
    const closedSignal = {
        ...currentSignal,
        status: result,
        closedAt: Date.now(), // Время закрытия для фильтрации
...
// Функция автоматической очистки истории старше 10 дней
function getLiveHistory() {
    const TEN_DAYS_MS = 10 * 24 * 60 * 60 * 1000;
    const now = Date.now();
    
    // Оставляем только те сигналы, которые закрыты менее 10 дней назад
