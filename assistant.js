...
// ====================================================================
// --- MEXC DATA ENGINE (ОБХОД БЛОКИРОВОК 451/403 ДЛЯ СЕРВЕРОВ США) ---
// ====================================================================

// MEXC API на 100% совпадает с логикой Binance, но не блокирует американские IP Render'а
...
async function fetchCandleData(symbol, interval = '15m', limit = 40) {
    try {
        ...
        // GOLD(XAU) пропускаем, чтобы он не выдавал ошибку (он есть только на форексе)
        if(mexcSymbol === 'XAUUSDT' || mexcSymbol === 'GOLD') return null;
...
async function updateHtfLevels() {
    ...
    for (const asset of ASSETS) {
        ...
        try {
            // У MEXC интервал "1h" сокращается до "60m", используем его для надежности
        ...
        // Задержка 150мс между запросами для обхода лимитов
        await new Promise(r => setTimeout(r, 150));
    }
    if (successCount > 0) {
        console.log(`✅ [System]: HTF Levels Updated for ${successCount} assets via MEXC API.`);
    }
}

// --- BINANCE FUTURES WS ENGINE (Он успешно работает, оставляем!) ---
