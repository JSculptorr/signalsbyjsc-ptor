...
async function processSurgicalRadar() {
    // 1. INSTITUTIONAL KILL-ZONE (10:00 - 20:00 MSK -> 07:00 - 17:00 UTC)
    ...
    for (const coin of ASSETS) {
        try {
            ...
            if (struct.h4Trend === 'SIDEWAYS') continue; // Пропускаем, если тренд H4 не ясен
            ...
            // 2. ДЕТЕКТОР СВИПА С УЧЕТОМ H4 TREND
            if (price > struct.htfHigh && !struct.swept && struct.htfHigh > 0) {
                if (struct.h4Trend !== 'DOWN') continue; // Только шорт при медвежьем тренде
                struct.swept = 'SHORT';
                broadcastHackerLog(`${coin.id}: Ликвидность сверху снята. Тренд H4: DOWN.`, 'SWEEP');
            } else if (price < struct.htfLow && !struct.swept && struct.htfLow > 0) {
                if (struct.h4Trend !== 'UP') continue; // Только лонг при бычьем тренде
                struct.swept = 'LONG';
                broadcastHackerLog(`${coin.id}: Ликвидность снизу снята. Тренд H4: UP.`, 'SWEEP');
            }
            ...
            if (struct.swept) {
                ...
                if (isImpulsive) {
                    ...
                    if (hasFVG) {
                        broadcastHackerLog(`${coin.id}: Surgical Entry MSS + FVG (RR 1:3)`, 'MSS');
...
async function createSurgicalSignal(coin, price, side) {
    ...
    const tp = side === 'LONG' ? price + slDist * 3 : price - slDist * 3; // RR 1:3
    ...
    const newSig = new ActiveSignal({
        ...
        entry: price, sl, tp, size: posSize, desc: `SMC Logic | H4 Guard | RR 1:3`
    });
...
function checkTradeExecution(coin, price) {
    ...
    if (progressPercent >= 40 && !sig.partialHit) {
        ...
        broadcastHackerLog(`${coin.id}: 40% цели взято. БЕЗУБЫТОК.`, 'INFO');
    }
...
// ИСПРАВЛЕНИЕ: ПРОКСИ ДАННЫХ ДЛЯ ТОРГОВЫХ ГРАФИКОВ
...
server.listen(PORT, () => {
    ...
    broadcastHackerLog("Ядро Surgeon Ultimate v6.1 загружено (H4 Guard + 1:3 RR)", "INFO");
});