...
function analyzePriceAction(price, ema, rsi) {
    ...
    if (!currentSignal) {
        ...
        // ПОКУПКА (BUY): Цена у уровня, цена выше EMA (тренд вверх), RSI не перегрет (< 70)
        if ((nearResistance || nearSupport) && price > ema && rsi < 70) {
            type = "BUY";
            entry = price;
        } 
        // ПРОДАЖА (SELL): Цена у уровня, цена ниже EMA (тренд вниз), RSI не перепродан (> 30)
...
async function updatePrice() {
    try {
        // ИСПОЛЬЗУЕМ COINBASE EXCHANGE API (история свечей, не блокируется)
        // granularity=900 это 15-минутные свечи
        const res = await axios.get('https://api.exchange.coinbase.com/products/BTC-USD/candles?granularity=900');
        
        // Coinbase возвращает массив от новых к старым. Нам нужно развернуть его (reverse), 
        // и взять цену закрытия (индекс 4). Ограничиваемся 100 свечами.
        const closes = res.data.slice(0, 100).map(candle => parseFloat(candle[4])).reverse();
        
        if (closes.length > 0) {
            lastPrice = closes[closes.length - 1]; // Текущая цена
            const ema = calculateEMA(closes, 20); // Считаем тренд
            const rsi = calculateRSI(closes, 14); // Считаем силу

            analyzePriceAction(lastPrice, ema, rsi);
        }
    } catch (e) { 
        console.error("Coinbase Advanced API Error:", e.message); 
        
        // Надежный Fallback на старый метод, если свечи почему-то не отдаются
        try {
            const fb = await axios.get('https://api.coinbase.com/v2/prices/BTC-USDT/spot');
            lastPrice = parseFloat(fb.data.data.amount);
            // Если нет свечей, ставим нейтральные индикаторы
            analyzePriceAction(lastPrice, lastPrice, 50); 
        } catch (err) {
            console.error("Coinbase Fallback Error:", err.message);
        }
    }
}
