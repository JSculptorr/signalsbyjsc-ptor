...
const connectDB = async () => {
    try {
        ...
        console.log("✅ [SUCCESS]: Хирург v6.2 (Adaptive Multi-Asset) подключен к БД");
        syncWithDatabase();
...
// --- ЯДРО АНАЛИЗА BINANCE (ADAPTIVE V6.2) ---
async function processSurgicalRadar() {
    const hour = new Date().getUTCHours();
    const isKillZone = hour >= 7 && hour < 17; // Лондон + Нью-Йорк
    ...
    for (const coin of ASSETS) {
        try {
            ...
            // Если бот долго не видит тренда, он пропускает
            ...
            // 1. ДЕТЕКТОР СВИПА (С учетом адаптивного тренда)
            if (price > struct.htfHigh && !struct.swept && struct.htfHigh > 0) {
                if (struct.h4Trend === 'DOWN' || struct.h4Trend === 'WEAK_DOWN') {
                    struct.swept = 'SHORT';
                    broadcastHackerLog(`${coin.id}: Свип сверху (Liquidity Grab). Контекст: ${struct.h4Trend}`, 'SWEEP');
                }
            } else if (price < struct.htfLow && !struct.swept && struct.htfLow > 0) {
                if (struct.h4Trend === 'UP' || struct.h4Trend === 'WEAK_UP') {
                    struct.swept = 'LONG';
                    broadcastHackerLog(`${coin.id}: Свип снизу (Liquidity Grab). Контекст: ${struct.h4Trend}`, 'SWEEP');
                }
            }

            // 2. ФИЛЬТРЫ ВХОДА (MSS + FVG)
            if (struct.swept) {
                ...
                // Импульс должен быть заметным
                if (bodySize > (avgBody * 1.3)) {
                    let hasFVG = false;
...
async function createSurgicalSignal(coin, price, side) {
    ...
    const newSig = new ActiveSignal({
        ...
        entry: price, sl, tp, size: posSize, desc: `SMC Adaptive v6.2 | RR 1:2`
    });
...
// ОБНОВЛЕННАЯ ЛОГИКА ТРЕНДА H4 (АДАПТИВНАЯ)
async function updateHtfLevels() {
    for (const asset of ASSETS) {
        try {
            ...
            struct.htfHigh = Math.max(...candles.map(d => parseFloat(d[2])));
            struct.htfLow = Math.min(...candles.map(d => parseFloat(d[3])));
            ...
            const firstOpen = parseFloat(h4Candles[0][1]);
            const lastClose = parseFloat(h4Candles[h4Candles.length - 1][4]);

            // Новая логика: 3 из 5 достаточно, если общая цена выросла/упала
            if (upCount >= 3 && lastClose > firstOpen) {
                struct.h4Trend = 'UP';
            } else if (downCount >= 3 && lastClose < firstOpen) {
                struct.h4Trend = 'DOWN';
            } else if (upCount >= 4 || downCount >= 4) {
                struct.h4Trend = upCount >= 4 ? 'UP' : 'DOWN';
            } else {
                struct.h4Trend = 'SIDEWAYS';
            }
            
            broadcastHackerLog(`${asset.id}: Trend H4 [${struct.h4Trend}]`, 'INFO');
        } catch (e) { }
    }
}
...
function checkTradeExecution(coin, price) {
    ...
    if (progressPercent >= 40 && !sig.partialHit) {
        ...
        broadcastHackerLog(`${coin.id}: 40% цели взято. Стоп в БЕЗУБЫТОК.`, 'INFO');
    }
...
server.listen(PORT, () => {
    console.log(`📡 Surgical Adaptive v6.2 (24 Assets) Live on port ${PORT}`);
});