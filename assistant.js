...
const connectDB = async () => {
    try {
        ...
        console.log("✅ [TITAN FLOW]: Institutional Master Engine v8.7 Online");
        syncWithDatabase();
    } catch (err) { 
...
async function scoringEngine(coinId, price) {
    const tracker = orderFlowTracker[coinId];
    if (activeMasterSignals[coinId]) return; 
    ...
    if (clusterVol > 0 && pRange < price * 0.0002) { score += 30; reasons.push("ABSORPTION"); }

    if ((tracker.deltaVelocity > 0 && tracker.sweepSide === 'LONG') || (tracker.deltaVelocity < 0 && tracker.sweepSide === 'SHORT')) {
        score += 20; reasons.push("DELTA_MOMENTUM");
    }
...
server.listen(PORT, () => {
    console.log(`\n=========================================`);
    console.log(` >>> TITAN FLOW MASTER v8.7 READY <<< `);
    console.log(`=========================================`);
});