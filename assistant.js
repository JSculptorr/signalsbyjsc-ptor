...
async function updateMarket() {
    for (const coin of ASSETS) {
        try {
            ...
            const candles = res.data.slice(0, 100).reverse(); 
            ...
            console.error(`Coinbase API Error for ${coin.id}:`, e.message); 
            try {
...
app.get('/api/data', async (req, res) => {
    ...
    // ЛОГИКА: Активные сигналы только для Премиум. Если нет премиума - пустой список.
    const processedActive = isPrem ? active : [];

    res.json({ 
        prices: currentPrices, 
        activeSignals: processedActive, 
        // ИСТОРИЯ ТЕПЕРЬ ДЛЯ ВСЕХ (ДЛЯ РЕКЛАМЫ)
