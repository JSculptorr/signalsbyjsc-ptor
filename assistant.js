...
// 🛡️ БРОНЯ ОТ НЕВИДИМЫХ ПРОБЕЛОВ И ОШИБОК КОПИРОВАНИЯ В RENDER
const CONFIG = {
    API_KEY_GEMINI: (process.env.API_KEY_GEMINI || "").trim(),
    GEMINI_MODEL: (process.env.GEMINI_MODEL || "gemini-1.5-flash").trim().replace(/['"]+/g, ''),
...
async function runMarketScan() {
    ...
    // 🛡️ ТЕХНИЧЕСКИЙ ФИЛЬТР (v3.0): Просыпаемся только при сильном сетапе (score >= 45)
    const candidates = Object.keys(orderFlowTracker)
        .filter(id => orderFlowTracker[id].currentScore >= 45 && (Date.now() - lastAiAnalysis[id] > 240000))
    ...
    for (const id of candidates) {
        if(id === 'GOLD') continue; 
        const candles = await fetchCandleData(id, '15m', 100); // 🛡️ ПОВЫШЕННЫЙ КОНТЕКСТ (v2.0)
    ...
    try {
        // 🚀 НОВЫЙ ПРОМПТ: SENIOR INSTITUTIONAL RISK MANAGER (v2.0)
        ...
        STRICT RULES:
        1. ANTI-CHASING: If the price has already moved more than 3% in the last 2 hours, DO NOT enter the trend. It is likely a climax top/bottom.
        2. LIQUIDITY FIRST: Only enter if you see a clear "Liquidity Sweep" (wicking below previous lows/highs) BEFORE the move.
        3. STOP LOSS LOGIC: Do not use fixed 1%. Set SL below the TRUE structural low/high (Order Block). If the risk is more than 2%, skip the trade.
        4. SELF-LEARNING: Look at the last FAILED trades below. Analysis: Why did they fail? (Likely late entry or tight SL). DO NOT REPEAT THESE MISTAKES.

        [SELF-LEARNING MEMORY - Твои последние 5 сделок]:
        ${learningContext}

        Market Data for Analysis (100 candles context): 
        ${JSON.stringify(batchData)}
        
        OUTPUT ONLY JSON ARRAY (no markdown). Format:
        [{"id": "BTC", "action": "EXECUTE_LONG", "strategy": "SMC", "reason": "...", "confidence": 92, "tp": 0, "sl": 0}]
        Be extremely picky. 90% of setups should be "WATCH".`;
        ...
        for (const v of verdicts) {
            ...
            if (v.action.includes('EXECUTE') && v.confidence >= 85 && !activeMasterSignals[v.id]) { // 🛡️ ПОВЫШЕННЫЙ ПОРОГ ВХОДА (v2.0)
...
// 🛡️ ИНТЕРВАЛ ЭКОНОМИИ (v3.0): 300000ms = 5 минут.
setInterval(runMarketScan, 300000); 
