...
async function runMarketScan() {
    ...
    // 🛡️ ТЕХНИЧЕСКИЙ ФИЛЬТР (v4.0): Просыпаемся только при сильном сетапе (score >= 45)
    ...
    for (const id of candidates) {
        if(id === 'GOLD') continue; 
        // 🔬 ГЛУБОКОЕ ЗРЕНИЕ (v4.0): Запрашиваем HTF (1h) и LTF (15m) контекст
        const candles1h = await fetchCandleData(id, '1h', 100);  // 100 свечей H1 (>4 дней)
        const candles15m = await fetchCandleData(id, '15m', 250); // 250 свечей M15 (~3 дня)
        
        if (candles1h && candles15m) {
            batchData.push({ 
                id, 
                price: currentPrices[id], 
                h1_context_htf: candles1h, 
                m15_context_ltf: candles15m 
            });
        }
    }
    ...
    try {
        // 🚀 НОВЫЙ ПРОМПТ: SENIOR INSTITUTIONAL RISK MANAGER (v4.0 DOUBLE VISION)
        ...
        STRICT RULES (MTF ANALYSIS):
        1. HTF ALIGNMENT: Check the H1 context first. Identify major Order Blocks and Trend. NEVER go against the H1 trend.
        2. ANTI-CHASING: If the price has already moved more than 3% in the last 2 hours, DO NOT enter.
        3. LIQUIDITY FIRST: Only enter if you see a clear "Liquidity Sweep" on M15 BEFORE the move.
        4. STOP LOSS LOGIC: Set SL below the TRUE structural low/high on M15. If risk > 2%, skip.
        5. SELF-LEARNING: Look at the last FAILED trades. DO NOT REPEAT THESE MISTAKES.

        [SELF-LEARNING MEMORY - Твои последние 5 сделок]:
        ${learningContext}

        Market Data (H1 + M15 Context): 
        ${JSON.stringify(batchData)}
        
        OUTPUT ONLY JSON ARRAY. Format:
        [{"id": "BTC", "action": "EXECUTE_LONG", "strategy": "SMC", "reason": "...", "confidence": 92, "tp": 0, "sl": 0}]
        Be extremely picky. If H1 and M15 don't align, output "WATCH".`;
...
// 🛡️ ИНТЕРВАЛ ЭКОНОМИИ (v4.0): 480000ms = 8 минут.
setInterval(runMarketScan, 480000); 
