    ...
    for (const coin of ASSETS) {
        try {
            ...
            if (price > struct.htfHigh && !struct.swept && struct.htfHigh > 0) {
                if (struct.h4Trend !== 'DOWN') continue; 
                struct.swept = 'SHORT';
                broadcastHackerLog(`${coin.id}: Свип сверху. Тренд H4: DOWN.`, 'SWEEP');
            } else if (price < struct.htfLow && !struct.swept && struct.htfLow > 0) {
                if (struct.h4Trend !== 'UP') continue; 
                struct.swept = 'LONG';
                broadcastHackerLog(`${coin.id}: Свип снизу. Тренд H4: UP.`, 'SWEEP');
            }
            ...
            if (struct.swept) {
                ...
                if (isImpulsive) {
                    ...
                    if (hasFVG) {
                        broadcastHackerLog(`${coin.id}: Слом MSS + FVG (RR 1:3)`, 'MSS');
...
async function createSurgicalSignal(coin, price, side) {
    ...
    const newSig = new ActiveSignal({
        ...
        // ИСПРАВЛЕНО: Добавлено название монеты в тип сигнала
...
// НОВЫЙ ЭНДПОИНТ: ПРОКСИ ДЛЯ ГРАФИКОВ (ИСПРАВЛЯЕТ SYNC ERROR)
...
// ПОДПИСКА НА PUSH (ДЛЯ КНОПКИ В ЛИЧНОМ КАБИНЕТЕ)
...
server.listen(PORT, () => {
    console.log(`\n=========================================`);
    console.log(` >>> JSCULPTOR ULTRA INFRASTRUCTURE v6.1 <<< `);
    console.log(`=========================================`);
    console.log(`📡 Surgical Core Live on port ${PORT}`);
});