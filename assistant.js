...
// --- СИСТЕМА АНТИ-БАНА BINANCE (FALLBACK MIRRORS) ---
const BINANCE_MIRRORS = [
    'https://fapi.binance.com',
    'https://fapi.binance.info' // Официальное зеркало для обхода блокировок IP
];
...
async function fetchWithFallback(endpoint) {
    for (const baseUrl of BINANCE_MIRRORS) {
        try {
            ...
            continue; // Если один сервер заблокировал, пробуем следующий
        }
    }
    throw new Error("All Binance mirrors blocked");
}

async function fetchCandleData(symbol, interval = '15m', limit = 40) {
    try {
        const binanceSymbol = symbol.toUpperCase().replace('/', '');
        const endpoint = `/fapi/v1/klines?symbol=${binanceSymbol}&interval=${interval}&limit=${limit}`;
...
async function updateHtfLevels() {
    ...
    for (const asset of ASSETS) {
        try {
            const endpoint = `/fapi/v1/klines?symbol=${asset.symbol.toUpperCase()}&interval=1h&limit=24`;
            const data = await fetchWithFallback(endpoint);
            ...
            console.error(`❌ [Error]: Levels fail for ${asset.id} (Render IP Blocked)`);
        }
        // Микро-задержка для защиты от бана (Anti-Spam)
        await new Promise(r => setTimeout(r, 100));
    }
    if (successCount > 0) {
        console.log(`✅ [System]: HTF Levels Updated for ${successCount}/${ASSETS.length} assets via Mirrors.`);
    }
}
