...
// --- КОНФИГУРАЦИЯ АКТИВОВ (РАСШИРЕННАЯ ДО 10 МОНЕТ) ---
const ASSETS = [
    { id: 'BTC', pair: 'BTC-USD' },
    { id: 'ETH', pair: 'ETH-USD' },
    { id: 'SOL', pair: 'SOL-USD' },
    { id: 'XRP', pair: 'XRP-USD' },
    { id: 'LINK', pair: 'LINK-USD' },
    { id: 'ADA', pair: 'ADA-USD' },
    { id: 'DOT', pair: 'DOT-USD' },
    { id: 'DOGE', pair: 'DOGE-USD' },
    { id: 'AVAX', pair: 'AVAX-USD' },
    { id: 'MATIC', pair: 'MATIC-USD' }
];

let currentPrices = { BTC: 0, ETH: 0, SOL: 0, XRP: 0, LINK: 0, ADA: 0, DOT: 0, DOGE: 0, AVAX: 0, MATIC: 0 };
let activeSignals = { BTC: null, ETH: null, SOL: null, XRP: null, LINK: null, ADA: null, DOT: null, DOGE: null, AVAX: null, MATIC: null }; 
let tradeHistory = []; 
let stats = { total: 0, wins: 0, losses: 0, winRate: 0, maxDrawdown: 0, avgRR: 0, streak: 0 };
let antiTilt = { 
    BTC: { losses: 0, pauseUntil: 0 }, 
    ETH: { losses: 0, pauseUntil: 0 }, 
    SOL: { losses: 0, pauseUntil: 0 },
    XRP: { losses: 0, pauseUntil: 0 },
    LINK: { losses: 0, pauseUntil: 0 },
    ADA: { losses: 0, pauseUntil: 0 },
    DOT: { losses: 0, pauseUntil: 0 },
    DOGE: { losses: 0, pauseUntil: 0 },
    AVAX: { losses: 0, pauseUntil: 0 },
    MATIC: { losses: 0, pauseUntil: 0 }
};
...
function findInstitutionalOB(ticks, type) {
    ...
    for (let i = ticks.length - 10; i < ticks.length - 2; i++) {
        ...
        const displacement = Math.abs(next[4] - next[1]) > avgBody * 2.2; 
...
async function analyzeSMC_Ultimate(coin, h1ticks) {
    ...
    } catch (e) { console.error(`Ошибка анализа ${coin.id}`); }
}
...
app.listen(PORT, () => {
    ...
    console.log(`=========================================`);
    console.log(`[OK] ASSETS LOADED: 10 HIGH-LIQUID COINS`);
    console.log(`[OK] 1% RISK & POSITION SIZING: ACTIVE`);
    console.log(`=========================================\n`);
});