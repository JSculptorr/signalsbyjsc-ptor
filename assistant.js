...
const SIGNAL_SPACING_MS = 45 * 60 * 1000;
const COIN_SIGNAL_COOLDOWN_MS = 4 * 60 * 60 * 1000;
const LOSS_HOLD_MS = 6 * 60 * 60 * 1000;
const CLUSTER_WINDOW_MS = 90 * 60 * 1000;
const MAX_CLUSTER_SIGNALS = 1;
const MAX_DAILY_FAILED_TRADES = 2;
...
function isOwnerAccount(user) {
    return !!(user && user.email && (user.email === 'JSculptor-TITAN' || user.email.includes('JSculptor-TITAN')));
}

async function resolvePremiumAccess(user) {
    if (!user) return { isPrem: false, expiresAt: null };
    if (isOwnerAccount(user)) return { isPrem: true, expiresAt: user.subscriptionExpires || null };

    const expiresAt = user.subscriptionExpires ? new Date(user.subscriptionExpires) : null;
    const hasValidExpiry = expiresAt && Number.isFinite(expiresAt.getTime()) && expiresAt > new Date();
    const isPrem = user.subscriptionStatus === 'active' && hasValidExpiry;

    if (user.subscriptionStatus === 'active' && !isPrem) {
        user.subscriptionStatus = 'inactive';
        user.subscriptionExpires = null;
        await user.save();
        console.log(`[ACCESS DENIED]: Подписка пользователя ${user.email} истекла или не имеет даты окончания.`);
    }

    return { isPrem, expiresAt: isPrem ? expiresAt : null };
}

async function expireExpiredSubscriptions() {
    try {
        const result = await User.updateMany(
            {
                subscriptionStatus: 'active',
                $or: [
                    { subscriptionExpires: { $lte: new Date() } },
                    { subscriptionExpires: null }
                ],
                email: { $not: /JSculptor-TITAN/i }
            },
            { $set: { subscriptionStatus: 'inactive', subscriptionExpires: null } }
        );

        if (result.modifiedCount > 0) {
            userCache = {};
            console.log(`[ACCESS CLEANUP]: Заморожено истекших аккаунтов: ${result.modifiedCount}`);
        }
    } catch (e) {
        console.error(`Subscription cleanup error: ${e.message}`);
    }
}
...
function resetRiskGovernorIfNeeded() {
    const dayKey = new Date().toISOString().slice(0, 10);
    if (riskGovernor.dayKey !== dayKey) {
        riskGovernor = {
            dayKey,
            lastSignalAt: 0,
            holdUntil: 0,
...
function getRiskHoldStatus() {
    resetRiskGovernorIfNeeded();
    const now = Date.now();
    if (riskGovernor.holdUntil > now) {
        return {
            hold: true,
            reason: riskGovernor.holdReason || 'Risk governor hold is active.',
            until: riskGovernor.holdUntil
        };
    }

    return { hold: false, reason: 'SECURE', until: 0 };
}

function canOpenSignal(coinId, side) {
    resetRiskGovernorIfNeeded();
    const now = Date.now();
    const holdStatus = getRiskHoldStatus();
    if (holdStatus.hold) return holdStatus;

    if (now - riskGovernor.lastSignalAt < SIGNAL_SPACING_MS) {
        return { hold: true, reason: 'Signal spacing active after previous entry.', until: riskGovernor.lastSignalAt + SIGNAL_SPACING_MS };
    }

    if (riskGovernor.coinCooldowns[coinId] && riskGovernor.coinCooldowns[coinId] > now) {
        return { hold: true, reason: `${coinId} cooldown after recent signal.`, until: riskGovernor.coinCooldowns[coinId] };
    }

    riskGovernor.recentSignals = riskGovernor.recentSignals.filter(sig => now - sig.timestamp < CLUSTER_WINDOW_MS);
    const sameCluster = riskGovernor.recentSignals.filter(sig => sig.side === side).length;
    if (sameCluster >= MAX_CLUSTER_SIGNALS) {
        return { hold: true, reason: 'Correlation cluster guard: same-direction signal already active recently.', until: now + SIGNAL_SPACING_MS };
    }

    return { hold: false, reason: 'SECURE', until: 0 };
}

function registerSignalOpen(coinId, side) {
    const now = Date.now();
    riskGovernor.lastSignalAt = now;
    riskGovernor.coinCooldowns[coinId] = now + COIN_SIGNAL_COOLDOWN_MS;
    riskGovernor.recentSignals.push({ coinId, side, timestamp: now });
}

function registerTradeResult(coinId, result) {
    resetRiskGovernorIfNeeded();
    const now = Date.now();
    if (result !== 'FAILED') return;

    riskGovernor.dailyFailures++;
    riskGovernor.coinCooldowns[coinId] = now + COIN_SIGNAL_COOLDOWN_MS;

    if (riskGovernor.dailyFailures >= MAX_DAILY_FAILED_TRADES) {
        riskGovernor.holdUntil = now + LOSS_HOLD_MS;
        riskGovernor.holdReason = `Daily loss guard: ${riskGovernor.dailyFailures} failed signals today.`;
    } else {
        riskGovernor.holdUntil = now + SIGNAL_SPACING_MS;
        riskGovernor.holdReason = `Cooldown after failed ${coinId} signal.`;
    }
}
...
async function createMasterSignal(coinId, entry, side, score, reason, tp, sl, rr = 2.2) {
    ...
    const riskStatus = canOpenSignal(coinId, side);
    if (riskStatus.hold) {
        broadcastHackerLog(`[Risk Governor]: ${coinId} ${side} blocked. ${riskStatus.reason}`, 'ALERT');
        return;
    }
    try {
        ...
        registerSignalOpen(coinId, side);
...
async function finalizeMasterTrade(coinId, result, exitPrice) {
    ...
    try {
        ...
        registerTradeResult(coinId, result);
...
app.get('/api/data', async (req, res) => {
        ...
        try {
            ...
            if (!userCache[userId] || (now - userCache[userId].lastUpdate > CACHE_TTL)) {
                ...
                if (user) {
                    ...
                    const access = await resolvePremiumAccess(user);
                    isPrem = access.isPrem;
                ...
                userCache[userId] = { isPrem, isOwner, expiresAt, statsData, tradeHistory, lastUpdate: now };
            } else {
    ...
    const riskStatus = getRiskHoldStatus();
    ...
    res.json({ 
        ...
        stats: statsData, 
        premium: isPrem, 
        subscriptionExpires: isPrem && userId && userCache[userId] ? userCache[userId].expiresAt : null,
        isOwner: isOwner, 
        ...
        risk_hold: riskStatus.hold,
        risk_reason: riskStatus.reason,
        risk_hold_until: riskStatus.until
    });
});
...
app.post('/api/activate', async (req, res) => {
    try {
        ...
        const codeDoc = await Code.findOne({ code: normalizedCode, isUsed: false });
        if (codeDoc) {
            const user = await User.findById(decoded.id);
            if (!user) return res.status(404).json({ error: "User not found" });

            const days = Math.max(1, parseInt(codeDoc.days, 10) || 30);
            const now = new Date();
            const currentExp = user.subscriptionExpires ? new Date(user.subscriptionExpires) : null;
            const baseDate = user.subscriptionStatus === 'active' && currentExp && currentExp > now ? currentExp : now;
            ...
            syncWithDatabase();
            res.json({ message: "Activated!", expiresAt: exp });
        } else res.status(404).json({ error: "Invalid or used code" });
    } catch (e) { res.status(401).json({ error: "Activation failed" }); }
});
...
setTimeout(expireExpiredSubscriptions, 15000);
setInterval(expireExpiredSubscriptions, 60 * 60 * 1000);
