...
const Binance = require('node-binance-api'); // ПОДКЛЮЧАЕМ СКScalpel
...
// --- ИСПОЛНИТЕЛЬ АВТО-ТОРГОВЛИ (BINANCE FUTURES 10x РЕАЛ) ---
    ...
    try {
        ...
        users.forEach(async (user) => {
            ...
            try {
                ...
                console.log(`🚀 [EXECUTING]: Real Trade for ${user.email} on ${symbol}`);
                
                // 1. Установка плеча 10x
                await binance.futuresLeverage(symbol, 10);
                
                // 2. Расчет количества (на 1% риска)
                // Для простоты берем минимальный лот или расчет из твоей логики
                ...
                console.log(`✅ [ORDER PLACED]: ${side} ${symbol} at ${signal.entry}`);
    ...
    try {
        ...
        if (score >= 8) {
            ...
            if (confirmed) {
                ...
                const currentBalance = user ? user.balance : 100; // По умолчанию
...
// ... (остальные функции уведомлений и финализации оставляем без изменений) ...
...
// ТУТ МЫ ПОЛУЧАЕМ ТВОИ 80$ ИЗ BINANCE
app.get('/api/data', async (req, res) => {
    ...
    if (auth && auth !== 'Bearer null') {
        try {
            ...
            if (user) {
                ...
                // Если есть ключи, тянем реальный баланс
                if (user.binanceKey && user.binanceSecret) {
                    ...
                    // Обновим баланс в базе, чтобы статистика была верной
                    user.balance = parseFloat(realBalance);
                    await user.save();
                }
            }
        } catch (e) { console.log("Data fetch error:", e.message); }
    }
    ...
    res.json({ 
        ...
        premium: isPrem, 
        autoTrade: autoMode,
        balance: realBalance // ОТПРАВЛЯЕМ БАЛАНС НА САЙТ
    });
});
