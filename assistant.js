...
// ====================================================================
// --- BYBIT DATA ENGINE (STEALTH MODE С МАСКИРОВКОЙ) ---
// ====================================================================

const BYBIT_REST = 'https://api.bytick.com'; // Теневое зеркало Bybit
const BYBIT_WS = 'wss://stream.bybit.com/v5/public/linear';

// Маскируемся под браузер Chrome, чтобы Cloudflare нас пропускал
const axiosConfig = {
    timeout: 8000,
    headers: {
...
async function fetchCandleData(symbol, interval = '15', limit = 40) {
    try {
        ...
        console.error(`[Error] Candle fetch fail for ${symbol}: ${e.message}`);
        return null;
    }
}
...
async function updateHtfLevels() {
    ...
    for (const asset of ASSETS) {
        try {
            ...
            console.error(`❌ [Error]: Levels fail for ${asset.id} (${e.message})`);
        }
        // Увеличенная задержка для безопасности от спам-фильтров
        await new Promise(r => setTimeout(r, 200));
    }
    if (successCount > 0) {
        console.log(`✅ [System]: HTF Levels Updated for ${successCount}/${ASSETS.length} assets via Bybit Stealth.`);
    }
}
