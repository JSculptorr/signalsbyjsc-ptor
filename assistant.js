...
function analyzePriceAction(coin, price, ema, rsi) {
    if (!price) return;
    ...
    for (const coin of ASSETS) {
        try {
            ...
            if (closes.length > 0) {
                ...
                analyzePriceAction(coin, currentPrices[coin.id], ema, rsi);
            }
        } catch (e) { 
            console.error(`Coinbase Advanced API Error for ${coin.id}:`, e.message); 
            
            // Надежный Fallback для каждой монеты
            try {
                ...
                analyzePriceAction(coin, currentPrices[coin.id], currentPrices[coin.id], 50); 
            } catch (err) {
...
app.get('/api/data', async (req, res) => {
    ...
    res.json({ 
        prices: currentPrices, // Отправляем объект с тремя ценами
...
app.listen(PORT, () => console.log(`🚀 Multi-Asset Engine Running on port ${PORT}`));