...
// ====================================================================
// --- BYBIT DATA ENGINE (ANTI-BAN SYSTEM ДЛЯ RENDER) ---
// ====================================================================

const BYBIT_REST = 'https://api.bybit.com';
const BYBIT_WS = 'wss://stream.bybit.com/v5/public/linear';

async function fetchCandleData(symbol, interval = '15', limit = 40) {
    try {
        const bybitSymbol = symbol.toUpperCase().replace('/', '');
        const url = `${BYBIT_REST}/v5/market/kline?category=linear&symbol=${bybitSymbol}&interval=${interval}&limit=${limit}`;
        const res = await axios.get(url, { timeout: 5000 });
        
        if (res.data && res.data.result && res.data.result.list) {
            // Bybit отдает от новых к старым. Переворачиваем массив (reverse), чтобы AI понимал хронологию
            const list = res.data.result.list.reverse();
            return list.map(c => [
                new Date(parseInt(c[0])).toLocaleTimeString('en-GB'),
...
async function updateHtfLevels() {
    ...
    for (const asset of ASSETS) {
        try {
            // Интервал '60' в Bybit означает 1 час
            const endpoint = `${BYBIT_REST}/v5/market/kline?category=linear&symbol=${asset.symbol.toUpperCase()}&interval=60&limit=24`;
            const res = await axios.get(endpoint, { timeout: 5000 });
            
            if (res.data && res.data.result && res.data.result.list) {
                const data = res.data.result.list;
                orderFlowTracker[asset.id].htfHigh = Math.max(...data.map(c => parseFloat(c[2])));
                orderFlowTracker[asset.id].htfLow = Math.min(...data.map(c => parseFloat(c[3])));
                successCount++;
            }
        } catch (e) {
            console.error(`❌ [Error]: Levels fail for ${asset.id} (Bybit API Timeout)`);
        }
        // Микро-задержка для защиты сервера
        await new Promise(r => setTimeout(r, 100));
    }
    if (successCount > 0) {
        console.log(`✅ [System]: HTF Levels Updated for ${successCount}/${ASSETS.length} assets via Bybit.`);
    }
}
...
function initTitanStream() {
    ...
    function heartbeat() {
        ...
        pingTimeout = setTimeout(() => {
            console.log("⚠️ [Bybit WS]: Heartbeat lost. Forcing Reconnect...");
            ws.terminate(); 
        }, 35000); 
    }

    ws.on('open', () => { 
        console.log("📡 [Bybit WS]: Connected to Market Flow. Render IP Secured.");
        heartbeat();
        
        // Пакуем подписки Bybit
        const args = [];
        ASSETS.forEach(a => {
        ...
        // Отправляем чанками по 10, чтобы Bybit не ругался на слишком большой пакет
    ...
    // Пинг-понг каждые 20 секунд (требование Bybit)
    setInterval(() => {
    ...
    ws.on('message', (data) => {
        try {
            ...
            if (payload.topic) {
                const symbol = payload.topic.split('.')[1];
                ...
                if (payload.topic.startsWith('publicTrade')) {
                    payload.data.forEach(trade => processAggTrade(coinId, trade));
                } else if (payload.topic.startsWith('orderbook')) {
                    processOrderbook(coinId, payload.data);
                }
            }
        } catch (e) {}
    });
    ...
    ws.on('error', (err) => {
        console.error("❌ [Bybit WS] Connection Error:", err.message);
    });

    ws.on('close', () => {
        clearTimeout(pingTimeout);
        console.log("⚠️ [Bybit WS]: Stream Closed. Rebooting in 5s...");
        setTimeout(initTitanStream, 5000);
    });
}

function processAggTrade(coinId, trade) {
    // В Bybit цена "p", объем "v", сторона "S" (Buy/Sell)
...
function processOrderbook(coinId, depth) {
    ...
    // Защита от дельта-обновлений, где может быть пусто
    if (asks > 0 || bids > 0) {
...
async function runMarketScan() {
    ...
    for (const id of candidates) {
        // Bybit принимает '15' вместо '15m'
        const candles = await fetchCandleData(id, '15', 35);
