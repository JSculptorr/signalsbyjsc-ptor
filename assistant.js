...
// --- ИСПОЛНИТЕЛЬ АВТО-ТОРГОВЛИ (BINANCE FUTURES 10x) ---
async function executeAutoTrades(signal) {
    try {
        ...
        users.forEach(async (user) => {
            try {
                const apiKey = decrypt(user.binanceKey, user.iv);
                const apiSecret = decrypt(user.binanceSecret, user.iv);

                const symbol = signal.coinId + 'USDT';
                const side = signal.type.includes('LONG') ? 'BUY' : 'SELL';
                
                // 1. Установка плеча 10x (Протокол Futures)
                console.log(`⚙️ [LEVERAGE]: Установка 10x для ${user.email} на ${symbol}`);
                
                // 2. Логика исполнения ордера
                // В этом блоке Хирург имитирует отправку на fapi.binance.com
                ...
                // Реальный вызов будет здесь (через axios с подписью HMAC SHA256)
                // const order = await placeFuturesOrder(apiKey, apiSecret, symbol, side, signal.size, signal.sl, signal.tp);

            } catch (err) { console.error(`Ошибка Auto-Trade на аккаунте ${user.email}`); }
        });
    ...
    try {
        ...
        if (score >= 8) {
            ...
            if (confirmed) {
                ...
                // ГОРЯЧИЙ ВЫЗОВ АВТО-ТОРГОВЛИ И ПУШЕЙ
...
app.get('/api/data', async (req, res) => {
    let isPrem = false; let autoMode = false;
    ...
    if (auth && auth !== 'Bearer null') {
        try {
            ...
            if (user) autoMode = user.autoTrade;
        } catch (e) {}
    }
    const active = Object.values(activeSignals).filter(s => s !== null);
    res.json({ prices: currentPrices, activeSignals: isPrem ? active : [], tradeHistory, stats, premium: isPrem, autoTrade: autoMode });
});
