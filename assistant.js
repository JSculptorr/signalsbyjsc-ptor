...
// --- СИСТЕМНЫЕ ПЕРЕМЕННЫЕ (TURBO LIMITS ENABLED) ---
let aiQueue = 0;      
let aiCallsLastMinute = 0;
const MAX_AI_CALLS_PER_MIN = 60; // ОБНОВЛЕНО: С 12 ДО 60 ДЛЯ ПЛАТНОЙ ВЕРСИИ
...
function processAggTrade(coinId, trade) {
    ...
    // ОБНОВЛЕНО: УВЕЛИЧЕНО ОКНО ПАМЯТИ ДО 60 СЕКУНД (60000 МС)
    tracker.absorptionBuffer = tracker.absorptionBuffer.filter(t => now - t.time < 60000);
...
// ОБНОВЛЕНО: ЧАСТОТА СКАНЕРА ТЕПЕРЬ РАЗ В 10 СЕКУНД
...
// --- API ROUTES (ПОЛНОСТЬЮ СОХРАНЕНЫ) ---
...
// ОБНОВЛЕНО: ДОБАВЛЕНЫ ЛОГИ ДЛЯ КОНТРОЛЯ УРОВНЕЙ HTF
async function updateHtfLevels() {
    for (const asset of ASSETS) {
        try {
            ...
            console.log(`[System]: Levels updated for ${asset.id} (H: ${orderFlowTracker[asset.id].htfHigh})`);
        } catch (e) {
            console.error(`[Error]: Levels fail for ${asset.id}`);
        }
    }
}
