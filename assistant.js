...
const { RestClientV5 } = require('bybit-api'); // ДОБАВЛЕНО ДЛЯ АВТОТОРГОВЛИ BYBIT
...
const User = mongoose.model('User', new mongoose.Schema({
    ...
    // --- НОВЫЕ ПОЛЯ ДЛЯ BYBIT ---
    bybitKey: { type: String, default: null },
    bybitSecret: { type: String, default: null },
    bybitIv: { type: String, default: null },
...
// ====================================================================
// --- 1. MEXC REST API ENGINE (Идеально для истории, обход бана) ---
// ====================================================================
const MEXC_API = 'https://api.mexc.com/api/v3';
const axiosConfig = { timeout: 8000, headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0)', 'Accept': 'application/json' } };

async function fetchCandleData(symbol, interval = '15m', limit = 40) {
    try {
        const mexcSymbol = symbol.toUpperCase().replace('/', '');
        if(mexcSymbol === 'XAUUSDT' || mexcSymbol === 'GOLD') return null; // Изолируем золото
...
// ====================================================================
// --- 2. BYBIT WEBSOCKET ENGINE (Идеально для очков и стакана) ---
// ====================================================================
...
function initTitanStream() {
    ...
    ws.on('open', () => { 
        console.log("📡 [Hybrid WS]: Connected to Bybit Market Flow. Scores active.");
        heartbeat();
        
        const args = [];
        // Подключаем только крипту, изолируем GOLD от Bybit WS во избежание сбоев
...
// --- ЛОГИКА АВТОТОРГОВЛИ BYBIT ---
async function executeAutoTrade(signal) {
    const users = await User.find({ autoTrade: true, subscriptionStatus: "active" });

    for (const user of users) {
        try {
            const key = decrypt(user.bybitKey, user.bybitIv);
            const secret = decrypt(user.bybitSecret, user.bybitIv);
            ...
            // 1. Устанавливаем плечо (например 10x)
            await client.setLeverage({ category: 'linear', symbol, buyLeverage: '10', sellLeverage: '10' });

            // 2. Открываем сделку (по рынку для мгновенного входа)
            ...
            console.log(`🎯 [AutoTrade]: Сделка открыта для ${user.email} по ${symbol}`);
        } catch (e) {
            console.error(`❌ [AutoTrade Error]: Ошибка у пользователя ${user.email}:`, e.message);
        }
    }
}
...
async function createMasterSignal(coinId, entry, side, score, reason, tp, sl) {
    ...
    try {
        ...
        // ВЫЗОВ АВТОТОРГОВЛИ ЗДЕСЬ
        executeAutoTrade(signal.toObject());
    } catch (e) {}
}
...
// --- НОВЫЙ МАРШРУТ ДЛЯ СОХРАНЕНИЯ КЛЮЧЕЙ BYBIT ---
app.post('/api/user/keys', async (req, res) => {
    try {
        ...
        await User.findByIdAndUpdate(decoded.id, {
            bybitKey: encryptedKey.encryptedData,
            bybitSecret: encryptedSecret.encryptedData,
            bybitIv: encryptedKey.iv,
            autoTrade: true
        });

        res.json({ message: "API Ключи сохранены и зашифрованы!" });
    } catch (e) {
