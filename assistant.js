...
async function createSurgicalSignal(coin, price, side) {
    ...
    const tp = side === 'LONG' ? price + slDist * 5 : price - slDist * 5; // RR 1:5
    ...
    const newSig = new ActiveSignal({
        ...
        entry: price, sl, tp, size: posSize, desc: `SMC Order Flow | RR 1:5`
    });
...
server.listen(PORT, () => {
    ...
    broadcastHackerLog("Ядро Surgeon Ultimate v6.0 загружено (High-Accuracy Mode)", "INFO");
});