...
const CONFIG = {
    ...
    GEMINI_MODEL: (process.env.GEMINI_MODEL || "gemini-2.5-flash").trim().replace(/['"]+/g, ''),
...
// 🎯 СНАЙПЕРСКИЕ ПРЕДОХРАНИТЕЛИ (НОВЫЕ ПЕРЕМЕННЫЕ)
let lastGlobalAiCall = 0; // Глобальный таймер (1 запрос в минуту)
const coinAiCooldowns = {}; // Индивидуальный бан монет на 30 мин
...
// ====================================================================
const MEXC_API = '[https://api.mexc.com/api/v3](https://api.mexc.com/api/v3)';
...
// ====================================================================
const BYBIT_WS = 'wss://[stream.bybit.com/v5/public/linear](https://stream.bybit.com/v5/public/linear)';
...
async function runMarketScan() {
    ...
    // 🛡️ ЗАЩИТА №1: Глобальный лимит (1 запрос в 60 сек на всю систему)
    if (Date.now() - lastGlobalAiCall < 60000) return;

    // 🛡️ ЗАЩИТА №2: Порог 80+ и Индивидуальный Cooldown
    const candidates = Object.keys(orderFlowTracker)
        .filter(id => {
            const tracker = orderFlowTracker[id];
            const isHighScoring = tracker.currentScore >= 80; // 🎯 ПОРОГ УСТАНОВЛЕН НА 80
        ...
        .slice(0, 1); // 🎯 БЕРЕМ ТОЛЬКО ОДНУ ЛУЧШУЮ МОНЕТУ В МИНУТУ
    ...
    // 🛡️ МАКСИМАЛЬНЫЙ КОНТЕКСТ (v4.5): 1000 H1 (41 день) + 1000 M15 (10 дней)
    const candles1h = await fetchCandleData(id, '1h', 1000);  
    const candles15m = await fetchCandleData(id, '15m', 1000); 
    ...
    try {
        // 🚀 НОВЫЙ ПРОМПТ ДЛЯ CSV: SENIOR INSTITUTIONAL RISK MANAGER
        ...
        1. DEEP HTF CHECK: Analyze the 1000 H1 candles. Identify major Liquidity Pools and Order Blocks from the last 30+ days.
        ...
        // 🛡️ ИСПРАВЛЕННЫЙ ПАРСИНГ: Используем безопасные символы, чтобы не ломать редактор
        ...
        for (const v of verdicts) {
            ...
            // 🛡️ ЗАЩИТА №3: Индивидуальный БАН на 30 минут если ИИ сказал "WATCH"
...
// --- ИСПРАВЛЕННАЯ ЛОГИКА ЗАВЕРШЕНИЯ СДЕЛКИ (ЗАЩИТА ОТ ДУБЛЕЙ) ---
function checkMasterExecution(coinId, price) {
    const sig = activeMasterSignals[coinId];
    if (!sig || sig.isProcessing) return; // 🛡️ Защита от дублей
...
app.post('/api/subscribe', async (req, res) => {
    try {
        ...
        // Проверяем, есть ли уже этот браузер в базе, чтобы не было дублей
...
// 🛡️ Оптимизированный роут с Кэшированием И ЖЕСТКОЙ ПРОВЕРКОЙ ПОДПИСКИ!
...
app.get('/api/klines', async (req, res) => {
    try {
        ...
        const mexcInterval = mapMexcInterval(interval); // И здесь тоже исправили
