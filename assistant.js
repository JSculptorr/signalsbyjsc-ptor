...
const connectDB = async () => {
    try {
        ...
        console.log("✅ [SUCCESS]: Хирург v6.2 (Adaptive Trend) подключен к БД");
        syncWithDatabase();
...
async function createSurgicalSignal(coin, price, side) {
    ...
    const newSig = new ActiveSignal({
        ...
        entry: price, sl, tp, size: posSize, desc: `SMC Adaptive v6.2 | RR 1:3`
    });
...
updateHtfLevels();
setInterval(updateHtfLevels, 900000); // 15 минут

const PORT = process.env.PORT || 10000;
server.listen(PORT, () => {
    console.log(`📡 Surgical Adaptive v6.2 Live on port ${PORT}`);
});