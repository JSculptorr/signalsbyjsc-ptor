...
// ====================================================================
// --- BINANCE SPOT DATA ENGINE (ОБХОД ГЕО-БЛОКИРОВКИ 403) ---
// ====================================================================

// Используем публичный спотовый API, который пропускает US IP сервера Render
...
async function fetchCandleData(symbol, interval = '15m', limit = 40) {
    try {
        ...
        // GOLD(XAU) нет на споте бинанса в чистом виде, ставим заглушку на PAXG если нужно, но пока пропускаем
        if(binanceSymbol === 'XAUUSDT') return null;

        const url = `${BINANCE_SPOT_API}/klines?symbol=${binanceSymbol}&interval=${interval}&limit=${limit}`;
...
async function updateHtfLevels() {
    ...
    for (const asset of ASSETS) {
        if(asset.id === 'GOLD') continue; // Пропускаем золото для спота

        try {
            const url = `${BINANCE_SPOT_API}/klines?symbol=${asset.symbol.toUpperCase()}&interval=1h&limit=24`;
            const res = await axios.get(url, { timeout: 5000 });
    ...
    if (successCount > 0) {
        console.log(`✅ [System]: HTF Levels Updated for ${successCount} assets via Binance Spot API.`);
    }
}

// --- BINANCE FUTURES WS ENGINE (Он работал без 403 ошибки!) ---
function initTitanStream() {
    ...
    ws.on('open', () => { 
        console.log("📡 [Binance WS]: Connected to Market Flow. Render IP Secured.");
        heartbeat();
    });
