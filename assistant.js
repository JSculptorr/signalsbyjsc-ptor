...
// --- VAPID PUSH NOTIFICATIONS (СОХРАНЕНО) ---
...
const connectDB = async () => {
    try {
        ...
        console.log("✅ [TITAN FLOW]: Institutional Engine v8.6 Online");
        syncWithDatabase();
    } catch (err) { 
...
// --- DATA MODELS (RETAINED & FIXED FOR USERID) ---
...
const ActiveSignal = mongoose.model('ActiveSignal', new mongoose.Schema({
    userId: { type: mongoose.Schema.Types.ObjectId, ref: 'User' }, // Убрали required для совместимости со старой базой
...
ASSETS.forEach(a => {
    orderFlowTracker[a.id] = {
        ...
        lastCVD: 0,
        deltaVelocity: 0,
        absorptionBuffer: [], // Trades cluster
        topBids: 0,
        topAsks: 0,
        htfHigh: 0,
        htfLow: 0,
...
// --- BINANCE WS ENGINE (V8.6 INSTITUTIONAL STREAM) ---
...
// 1. ADVANCED DELTA & CLUSTER ABSORPTION
function processAggTrade(coinId, trade) {
    ...
    // Absorption Cluster (5s Window) - Анализ серии сделок
    ...
    // Ведение сделок для каждого юзера отдельно
    if (activeSignals[coinId]) {
...
// 2. TOP-10 WEIGHTED IMBALANCE
function processOrderbook(coinId, depth) {
    const tracker = orderFlowTracker[coinId];
    // Анализируем только ТОП-10 лимитных уровней (реальная борьба)
...
// 3. SCORING ENGINE (v8.6 TITAN LOGIC)
async function scoringEngine(coinId, price) {
    const tracker = orderFlowTracker[coinId];
    if (activeSignals[coinId] && Object.keys(activeSignals[coinId]).length > 50) return; // Защита от перегрузки

    let score = 0;
    let reasons = [];

    // Factor 1: Liquidity Sweep (35 pts)
    if (tracker.sweepSide) {
        score += 35;
        reasons.push("LIQUIDITY_SWEEP");
    }

    // Factor 2: Absorption Cluster (30 pts)
    ...
    if (clusterVol > 0 && pRange < price * 0.0002) {
        score += 30;
        reasons.push("ABSORPTION_DETECTED");
    }

    // Factor 3: Delta Momentum (20 pts)
    if ((tracker.deltaVelocity > 0 && tracker.sweepSide === 'LONG') || 
        (tracker.deltaVelocity < 0 && tracker.sweepSide === 'SHORT')) {
        score += 20;
        reasons.push("DELTA_MOMENTUM");
    }

    // Factor 4: Top-Level Imbalance (15 pts)
...
// --- DB SYNC (FIXED FOR LEGACY DATA) ---
async function syncWithDatabase() {
    try {
        const dbActive = await ActiveSignal.find();
        dbActive.forEach(sig => { 
            // Защита: пропускаем сигналы без UserId (старые данные)
            if (sig.userId) {
...
app.get('/api/data', async (req, res) => {
    ...
    let userSignals = [];
    if (isPrem && userId && activeSignals) {
    ...
    res.json({ 
        prices: currentPrices, activeSignals: userSignals, 
        tradeHistory, stats, premium: isPrem, orderFlow: orderFlowTracker 
    });
});
...
server.listen(PORT, () => {
    console.log(`\n=========================================`);
    console.log(` >>> TITAN FLOW v8.6 INSTITUTIONAL <<< `);
    console.log(`=========================================`);
});